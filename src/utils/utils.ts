...
export const serviceOptions: { id: number; label: string }[] = Object.keys(
  ServiceType
)
  .filter(
    (key: string) =>
      !isNaN(Number(ServiceType[key as keyof typeof ServiceType]))
  )
  .map((key: string) => ({
...
// replacer all
export function stringFindAndReplaceAll(
  str: string,
  find: string,
  replace: string,
  position: number = 0
...
export const makeFileUrlFromBase64 = (
