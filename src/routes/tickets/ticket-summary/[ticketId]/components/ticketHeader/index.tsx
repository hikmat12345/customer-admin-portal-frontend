...
function TicketHeader({ ticketSummary, ticketSummaryLoading }: any) {
  ...
  function HeaderValue({ value, linkUrl }: { value: string, linkUrl? : string }) {
    return (
      <span className="pl-2 text-[0.813rem] font-[400] leading-[1.023rem] text-custom-dimGray w-3/4">
        {ticketSummaryLoading ? <Skeleton variant="paragraph" rows={1} /> : ticketSummary && value ? value && linkUrl ? 
        <Link className='text-custom-dryBlue text-[0.813rem] font-[400] leading-[1.06rem] visited:text-custom-dryBlue active:text-custom-dryBlue' href={linkUrl}>{value}</Link> : value : '-'}
      </span>
    );
  }
  ...
  return (
    <div className="grid w-full rounded-lg border border-custom-aluminum bg-custom-white p-4 sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 xl:px-6 2xl:px-12">
      <div className="flex items-center h-[2.6rem] p-3">
      ...
      <div className="flex items-center h-[2.6rem] p-3">
        ...
        <HeaderValue value={ticketSummary?.service?.serviceNumber} linkUrl={`/inventory/${ticketSummary?.service?.id}`} />
      </div>
      <div className="flex items-center h-[2.6rem] p-3">
        <HeaderLabel label="Employee" />
        <HeaderValue value={`${ticketSummary?.employee?.firstName} ${ticketSummary?.employee?.lastName}`} linkUrl={`/employees/${ticketSummary?.employee?.id}`} />
      </div>
      <div className="flex items-center h-[2.6rem] p-3">
        <HeaderLabel label="Vendor" />
        <HeaderValue value={ticketSummary?.companyNetwork?.displayName} linkUrl={`/vendors/${ticketSummary?.companyNetwork?.id}`} />
      </div>
    </div>
  );
}
