...
export default function InvoiceSummary({
    ...
    const staticData = [
        { label: 'Invoice Date', value: invoiceData.invoiceDate? formatDate(invoiceData.invoiceDate, 'MMM dd, yyyy') :"-" },
        ...
        { label: 'Date Entered', value: invoiceData.dateEntered? formatDate(invoiceData.dateEntered, 'MMM dd, yyyy') :"-"},
        { label: 'Country', value: invoiceData.country },
        { label: 'Invoice Due Date', value: invoiceData?.invoiceDueDate ? formatDate(invoiceData.invoiceDueDate, 'MMM dd, yyyy') : "-"  },
        ...
        <div>
            {isLoading ? (
                ...
                <div className="flex max-lg:block lg:gap-x-[32px] xl:gap-x-[60px] pb-6">
                    <div className="flex w-[29%] max-lg:w-[100%] max-lg:mt-5 justify-between lg:gap-x-[32px] xl:gap-x-[60px]">
                        <div className='w-[60%]'>
                            {staticData.slice(0, 9).map((item, index) => (
                                <div key={index} className="text-[#000] lg:text-[13px] xl:text-[14px] font-[600] leading-7">{item.label}</div>
                            ))}
                        </div>
                        <div className='w-[40%]'>
                            {staticData.slice(0, 9).map((item, index) => (
                                <div key={index}>
                                    {typeof item.value !== 'undefined' ? (
                                        typeof item.value === 'boolean' ? (
                                            ...
                                            <div className="text-custom-blue cursor-pointer">
                                                <button className="underline decoration-2 lg:text-[13px] xl:text-[14px] leading-7" onClick={() => { setShowInBrowser(true); fileDownloadFile(item.value, "pdf") }}>View</button>
                                            </div>
                                        ) : typeof item.value == "string" ? (
                                            <TooltipText
                                                text={item.value ? item.value : ' - '}
                                                maxLength={25}
                                                className="text-[#575757] lg:text-[13px] xl:text-[14px] leading-7"
                                            />
                                        ) :
                                            (
                                                <div>{item.value ? item.value : ' - '}</div>
                    ...
                    <div className="flex w-[29%] max-lg:w-[100%] max-lg:mt-5 justify-center lg:gap-x-[32px] xl:gap-x-[60px] pr-3">
                        <div className='w-[60%]'>
                    ...
                    <div className="w-[1px] bg-custom-aluminum h-[15rem] lg:block"></div>
                    <div className='  w-[41%] max-lg:mt-5'> 
                            ...
                            <VImage  src={process.env.NEXT_PUBLIC_ASSETS_LOGO_PATH + vendorData.logo} alt="Invoice Summary Logo" width={200} height={200} className="w-[390px] h-[108px] object-contain " />
                    </div>  
                </div>

                 <div className="flex gap-5 mb-4 mt-4">
                     <Button loading={isXlsFileLoading} type="submit" className="animate-in bg-transparent border-none flex items-center px-0 py-2 text-[#219653]  rounded hover:text-[#21965492]" onClick={() => fileDownloadFile(invoiceData?.invoiceId, "xls")}>
                        <Image src="/svg/excel-icon.svg" width={20} height={20} alt="Download Invoice Summary" className="mr-2" />
                        <span className="underline decoration-2 lg:text-[13px] xl:text-[16px] xl:leading-7 lg:leading-7 font-[600] w-[55%]">Download Invoice Summary </span>
                    </Button>
                    <Button loading={isPdfFileLoading} type="submit" className="animate-in bg-transparent border-none flex items-center px-4 py-2 text-[#E41323]  rounded hover:text-[#e4132499]" onClick={() => fileDownloadFile(invoiceData?.invoiceId, "pdf")}>
                        ...
                        <span className="underline decoration-2 lg:text-[13px] xl:text-[16px] xl:leading-7 lg:leading-7 font-[600] w-[55%]">Download PDF</span>
                    </Button>
                </div>
                </div>
            )}
