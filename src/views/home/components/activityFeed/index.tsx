...
export const ActivityFeed = ({ activityFeed, activityFeedLoading }: ActivityFeedProps) => (
  <>
    {activityFeedLoading ? (
      ...
      activityFeed?.map((activity: ActivityFeed, index: number) => {
        const imageUrl = makeFileUrlFromBase64(
          activity.activityFeedType?.logo.image
            ? Buffer.from(activity.activityFeedType?.logo.image).toString('base64')
            : null,
        );
