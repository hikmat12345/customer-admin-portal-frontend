...
const TotalTicketsOpen = ({ data, isLoading }: { data: OpenTicketSummary; isLoading: boolean }) => {
	...
	return (
		<div className="min-w-[300px] min-h-[150px] h-auto border border-custom-plaster pl-7 pt-3 rounded-lg relative">
			{isLoading ? (
				<div className="mt-2 lg:mr-7">{<Skeleton variant="paragraph" rows={3} className="mr-7" />} </div>
			) : (
				<div className="flex gap-[6px]">
					<div className="flex flex-col gap-4 pb-3 w-full pr-5">
						<h2 className="text-custom-black text-lg font-bold">Total Tickets Open</h2>
						<div className="flex items-center gap-5">
							<h1 className="text-lg lg:text-2xl 2xl:text-3xl font-bold text-custom-beer">{openTicketsCount}</h1>
						</div>
						<div className="flex items-center justify-between">
							...
							<div className="mt-[-100px] lg:mt-[-103px] 2xl:mt-[-103px] w-[120px] h-[103px] lg:h-[106px] 2xl:h-[120px] mr-2 relative">
								...
								<span className="absolute lg:bottom-[15%] xl:bottom-[23%] left-[34%] text-[8px] text-custom-greyBlue font-bold">
									in last 24 hours
								</span>
							</div>
						</div>
