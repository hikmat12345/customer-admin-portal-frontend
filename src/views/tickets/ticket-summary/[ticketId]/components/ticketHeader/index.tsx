    ...
    <div className="grid w-full rounded-lg border border-custom-aluminum bg-custom-white p-4 sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 xl:px-6 2xl:px-12">
      ...
      <div className="flex h-[2.6rem] items-center p-3">
        ...
        <div className="pl-2 sm:w-2/4 xl:w-3/4">
          {ticketSummaryLoading ? (
            ...
            <div
              className={`flex h-[2.313rem] w-[2.313rem] items-center justify-center rounded-full text-[0.813rem] font-[400] leading-[1.023rem] text-custom-white bg-[${
      ...
      <div className="flex h-[2.6rem] items-center p-3">
        ...
        <HeaderValue
          ...
          linkUrl={`/inventory/${ticketSummary?.service?.id}/${params.name}`}
        />
      </div>
      <div className="flex h-[2.6rem] items-center p-3">
      ...
      <div className="flex h-[2.6rem] items-center p-3">
        ...
        <HeaderValue
          value={
            ticketSummary?.companyNetwork?.network?.name && ticketSummary?.companyNetwork?.accountNumber
              ? `${ticketSummary?.companyNetwork?.network?.name} - ${ticketSummary?.companyNetwork?.accountNumber}`
              : '-'
          }
          linkUrl={`/vendors/${ticketSummary?.companyNetwork?.id}/${encrypt(ticketSummary?.companyNetwork?.network?.name)}`}
        />
      </div>
    </div>
  );
