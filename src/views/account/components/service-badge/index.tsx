...
export const ServiceTypeBadge = ({label="Mobile", count=35, color="", subTypes= []} : ServiceTypeBadgeProps) => {
    return (
        <div>
          <div className="w-[49%] h-[64px] bg-custom-background rounded border border-neutral-300 flex justify-between pl-5 pr-1 pt-2">
        ...
        <div className="flex gap-4 mt-4 flex-wrap  ">
            {subTypes.map(({name, service_type}, i) => {
                return <div className="w-[49%] h-[64px]  " key={i} >
                   <div className=" h-[64px] bg-custom-palePathaloBlue rounded border border-neutral-300 flex justify-between pl-5 pr-1 pt-2">
                        ...
                        <div className="inline-flex items-center justify-center text-blue-700 w-[60px] h-[44px] text-xl font-semibold  relative top-0 ">{service_type}</div>
                        </div>
                    </div> 
                </div>
            })}
