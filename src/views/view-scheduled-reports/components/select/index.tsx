      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="p-0 lg:w-[10rem] 2lg:w-[12.5rem] 2xl:w-[15.625rem]">
          <Command>
            {(index === 0 || index === 1) && (
              <>
                <CommandInput placeholder={PLACEHOLDER_NAME[index as 0 | 1]} />
                  ...
                  {menuOption?.options
                    ...
                    .map((option: any, indexCount: number) => (
                      <CommandItem
                        ...
                        value={option?.label}
                        onSelect={(currentValue) => {
                          // Find the selected option based on currentValue
                          const selectedOption = menuOption?.options
                            .flat()
                            .find((opt: any) => opt.label === currentValue);

                          if (selectedOption) {
                            // If currentParamValue already includes the selected value, deselect it
                            if (currentParamValue.includes(selectedOption.value.toString())) {
                              const paramIndex = paramValues.findIndex((param) => param == selectedOption.value);
                              paramValues.splice(paramIndex, 1);
                              const joinedParams = paramValues.join(',');
                              ...
                              // Single select logic: replace current value with the selected value
