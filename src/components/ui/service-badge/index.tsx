...
type ServiceTypeBadgeProps = {
    services: { servicType: string; count: number; subTypes: { name: string; servicType: number; count:number }[]; }[] 
} 

const ServiceTypesGrid = ({ services }: ServiceTypeBadgeProps) => {
    return (
        <div className="flex flex-wrap gap-4">
            {services.map((service: { servicType: string| number; count: number; subTypes: { name: string; servicType: number; count:number }[]; }, index: number) => (
                <> 
                <div key={index} className={` ${service.subTypes?.length > 1 ? 'w-[99%]' : 'w-[49%]'} h-[52px] bg-[#F4F7FE] rounded border border-neutral-300 flex  ${service.subTypes?.length  > 1 ? 'justify-center gap-3' : 'justify-between'} pl-5 pr-1 pt-1.5`}>
                    <div className="text-black pb-2 pt-2 text-base font-semibold">
                        {getServiceType(Number(service.servicType))}
                    </div>
                    <div className={`h-[41px] w-[41px] p-2 ${getServiceTypeColor(Number(service.servicType))} rounded-[41px] justify-start items-center gap-2.5 inline-flex pb-0  !pt-0`}>
                ...
                {service.subTypes.map((subType: { name: string; servicType: number; count: number }, subIndex: number) => (
                    <div key={subIndex} className={`  ${service.subTypes?.length > 2 ? "w-[32.2%]": "w-[49%]"} h-[52px] ${getServiceTypeSubColor(Number(service.servicType))} rounded border border-neutral-300 flex justify-between pl-5 pr-1 pt-2`}>
