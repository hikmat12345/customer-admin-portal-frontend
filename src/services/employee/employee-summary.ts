...
export const getEmployeeDetail = async ({ queryKey }: any) => {
  ...
  return httpClient.get(`${NEXT_PUBLIC_API_BASE_URL}/employee/detail/${employeeId}`).then(({ data }) => data).catch((error) => error);
};
...
export const getEmployeeServiceTypes = async ({ queryKey }: any) => {
  ...
  return httpClient.get(`${NEXT_PUBLIC_INVENTORY_SERVICE_URL}/service-types-employee/${employeeId}`).then(({ data }) => data).catch((error) => error);
};
// getEmployeeServices
...
export const getAllEmployees = async ({ queryKey }: any) => {
  ...
  return httpClient.get(`${NEXT_PUBLIC_INVENTORY_SERVICE_URL}/employees`, config).then(({ data }) => data).catch((error) => error);
};
